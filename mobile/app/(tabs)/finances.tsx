...
export default function DashboardScreen() {
  ...
  if (isPending && !data) {
    return (
      <View style={{ flex: 1, backgroundColor: ld.bg }}>
        <FinancesAppBar />
        <FinancesSubNav active="finances" />
  ...
  return (
    <View style={{ flex: 1, backgroundColor: ld.bg }}>
      <FinancesAppBar subtitle={data?.meta.subtitle || 'Overview'} />
      <FinancesSubNav active="finances" />
      <ScrollView
        style={{ flex: 1, opacity: isFetching ? 0.9 : 1 }}
        contentContainerStyle={{ padding: 16, paddingBottom: 40 }}
        refreshControl={<RefreshControl refreshing={isFetching && !isPending} onRefresh={onRefresh} />}
      >
      <View className="flex-row items-end justify-between mb-4">
        <View className="flex-1 pr-3">
          <Text style={{ color: ld.dim, fontSize: 12.5 }}>{data.meta.periodLabel || data.meta.subtitle}</Text>
        </View>
        <TouchableOpacity
